...
//exercise 1
let emailCompanyArr = users.map(u => {
    return {email: u.email, companyName: u.company.name}
});
console.log(emailCompanyArr);

//exercise 2
let zipCodeArr = users.filter(u => u.address.zipcode[0] === "5");
console.log(zipCodeArr);

//exercise 3
let zipCodeIdArr = users.filter((u) => u.address.zipcode[0] === "5").map(u => u.id);
console.log(zipCodeIdArr);

//exercise 4
let namesWithC = users.map(u => u.name).filter(u => u[0] === "C")
...
//exercise 6
let findUser = users.find(u => u.address.suite === "Apt. 950");
